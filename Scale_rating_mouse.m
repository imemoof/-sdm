...
%                 [rating_x,rating_y,buttons] = GetMouse();  %waits for a key-press
%                 [keyIsDown, secs, keyCode, deltaSecs] = KbCheck(-1);
%                 MousePress = any(buttons); %sets to 1 if a button was pressed
%                 if keyIsDown && keyCode(key.space)== 1; 
%                     MousePress = 100;
%                     isvalidate = 1;
%                 elseif keyIsDown && keyCode(key.escape) == 1
%                     stoptask = 1;
%                 end
% 
%             end
...
%                            rating_x = rating_x - 1920;  % a weird screen coordination system
%                        end
        ...
        % if the click falls in a certain range of x and y, update xCursor
         if rating_x > (displayConfig.xCenter - scale.half_x - 10) && rating_x < (displayConfig.xCenter + scale.half_x + 10) &&  rating_y >  (displayConfig.yCenter + scale.position_y - 20) && rating_y <  (displayConfig.yCenter + scale.position_y + 20)
            if rating_x < (displayConfig.xCenter - scale.half_x)
                xCursor = displayConfig.xCenter - scale.half_x;
            elseif rating_x > (displayConfig.xCenter + scale.half_x)
                xCursor = displayConfig.xCenter + scale.half_x;
            else
                xCursor = rating_x;
            end
            ...
            %=========== Refresh the screen after click
