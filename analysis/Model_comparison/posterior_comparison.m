...
m4_choice = load('sdm_model_fit_m_10.mat')
...
obs_m4 = safepos(m4_choice.params(1).Val_param_obs(subject));

% get the evolution parameter posterior
evo_m2 = m2_choice.params(1).Val_param_evo(subject);
evo_m4_bonus = m4_choice.params(1).Val_param_evo(subject,1);
evo_m4_beta = safepos(m4_choice.params(1).Val_param_evo(subject,2));
...
m4_evidence = m4_choice.params(1).Rating_model_evidence(subject);
...
[posterior,out] = VBA_groupBMC([m1_evidence, m2_evidence,m3_evidence,m4_evidence]')
[posterior,out] = VBA_groupBMC([m2_evidence,m3_evidence,m4_evidence]')

% plot the posterior of observation parameter
figure
hold on, set(gca,'fontsize',20)%,ylim([0.75, 1])
bar([mean(obs_m1),mean(obs_m2),mean(obs_m3),mean(obs_m4)])
errorbar([mean(obs_m1),mean(obs_m2),mean(obs_m3),mean(obs_m4)],[nanstd(obs_m1)/sqrt(length(subject)), nanstd(obs_m2)/sqrt(length(subject)),nanstd(obs_m3)/sqrt(length(subject)),nanstd(obs_m4)/sqrt(length(subject))],'.');
...
figure
boxplot([obs_m1,obs_m2,obs_m3,obs_m4])
...
% plot the posterior of the evolution parameter for model 2
figure
boxplot([evo_m2])
title('model 2 fit to choice','FontSize',20)
xlabel ('model index','FontSize',20)
ylabel('bonus for winning in pairwise comparison','FontSize',20)

% plot the posterior of the evolution parameter for model 4
figure 
subplot(1,2,1)
boxplot([evo_m4_bonus])
title('model 4 fit to choice','FontSize',20)
xlabel ('model index','FontSize',20)
ylabel('bonus for winning in pairwise comparison','FontSize',20)
subplot(1,2,2)
boxplot([evo_m4_beta])
title('model 4 fit to choice','FontSize',20)
xlabel ('model index','FontSize',20)
ylabel('temprature for the bonus','FontSize',20)
...
figure
for i = 1:length(subject)
    x_m4 = m4_choice.params(subject(i)).Rating_updated;
    subplot(4,6,i)
    histogram(x_m4(:,1),hist_bins)
    hold on
    histogram(x_m4(:,360),hist_bins)    
    title(['subject ',num2str(i)])
    xlabel('value range')
    ylabel('frequency')
...
m4_rt = load('sdm_rt_model_fit_m_4.mat')
...
obs_m4_beta = safepos(m4_rt.params(1).Val_param_obs(subject,1));
...
obs_m4_ke = m4_rt.params(1).Val_param_obs(subject,2);
...
obs_m4_kc = m4_rt.params(1).Val_param_obs(subject,3);

evo_m2 = m2_rt.params(1).Val_param_evo(subject);
evo_m4_bonus = m4_rt.params(1).Val_param_evo(subject,1);
evo_m4_beta = safepos(m4_rt.params(1).Val_param_evo(subject,2));
...
m4_evidence = m4_rt.params(1).Rating_model_evidence(subject);
...
figure
hold on, set(gca,'fontsize',20)%,ylim([0.75, 1])
bar([mean(obs_m1_beta),mean(obs_m2_beta),mean(obs_m3_beta),mean(obs_m4_beta)])
errorbar([mean(obs_m1_beta),mean(obs_m2_beta),mean(obs_m3_beta),mean(obs_m4_beta)],[nanstd(obs_m1_beta)/sqrt(length(subject)), nanstd(obs_m2_beta)/sqrt(length(subject)),nanstd(obs_m3_beta)/sqrt(length(subject)),nanstd(obs_m4_beta)/sqrt(length(subject))],'.');
...
figure
boxplot([obs_m1_beta,obs_m2_beta,obs_m3_beta,obs_m4_beta])
title('model fit to RT','FontSize',20)
xlabel ('model index','FontSize',20)
ylabel('inverse temp of obs function (safepos)','FontSize',20)
...
figure
hold on, set(gca,'fontsize',20)%,ylim([0.75, 1])
bar([mean(obs_m1_ke),mean(obs_m2_ke),mean(obs_m3_ke),mean(obs_m4_ke)])
errorbar([mean(obs_m1_ke),mean(obs_m2_ke),mean(obs_m3_ke),mean(obs_m4_ke)],[nanstd(obs_m1_ke)/sqrt(length(subject)), nanstd(obs_m2_ke)/sqrt(length(subject)),nanstd(obs_m3_ke)/sqrt(length(subject)),nanstd(obs_m4_ke)/sqrt(length(subject))],'.');
title('model fit to RT','FontSize',20)
xlabel ('model index','FontSize',20)
...
figure
boxplot([obs_m1_ke,obs_m2_ke,obs_m3_ke,obs_m4_ke])
title('model fit to RT','FontSize',20)
xlabel ('model index','FontSize',20)
ylabel('weight of the entropy','FontSize',20)
...
figure
hold on, set(gca,'fontsize',20)%,ylim([0.75, 1])
bar([mean(obs_m1_kc),mean(obs_m2_kc),mean(obs_m3_kc),mean(obs_m4_kc)])
errorbar([mean(obs_m1_kc),mean(obs_m2_kc),mean(obs_m3_kc),mean(obs_m4_kc)],[nanstd(obs_m1_kc)/sqrt(length(subject)), nanstd(obs_m2_kc)/sqrt(length(subject)),nanstd(obs_m3_kc)/sqrt(length(subject)),nanstd(obs_m4_kc)/sqrt(length(subject))],'.');
title('model fit to RT','FontSize',20)
xlabel ('model index','FontSize',20)
...
figure
boxplot([obs_m1_kc,obs_m2_kc,obs_m3_kc, obs_m4_kc])
title('model fit to RT','FontSize',20)
xlabel ('model index','FontSize',20)
ylabel('constant','FontSize',20)
...
subplot(1,2,1)
boxplot([evo_m4_bonus])
title('model 4 fit to RT','FontSize',20)
xlabel ('model index','FontSize',20)
ylabel('bonus for winning in pairwise comparison','FontSize',20)
subplot(1,2,2)
boxplot([evo_m4_beta])
...
figure
for i = 1:length(subject)
    x_m4 = m4_rt.params(subject(i)).Rating_updated;
    subplot(4,6,i)
    histogram(x_m4(:,1),hist_bins)
    hold on
    histogram(x_m4(:,360),hist_bins)   
